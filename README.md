...
`git-commit` stores its selected model profile and model under `~/.config/git-commit/config`. Run `git-commit configure` first, then run `git-commit` inside a Git repository to ask the configured model for a conventional-commit plan and print the `git add` and `git commit -m ...` commands for one or more commits from the current workspace changes. If staged changes already exist, it can optionally unstage them with `git restore --staged :/` before generating the preview. Use `--apply` to create the planned commits, and add `--push` to push them afterward.
