...
`git-profile` stores non-secret Git identity/config metadata under `${XDG_CONFIG_HOME:-$HOME/.config}/git-profile/<name>.gitconfig` and each optional GitHub PAT separately under `${XDG_DATA_HOME:-$HOME/.local/share}/git-profile/<name>.token`. The data directory is enforced to mode `0700` and PAT files to `0600`; this is filesystem-permission protection, not encryption. `git-profile set` applies the selected saved profile to the current repository and records the active profile name in local Git config as `picotools.gitProfile`. `git-profile clone <name> <url>` does the same while cloning. `git-profile token` resolves that repository-local marker and prints only the active profile PAT to stdout, so use it only in trusted pipelines or command substitution. `list` and `read` show only whether a PAT is configured and never print token bytes.
